...
| `headers` | object | `{}` | Custom HTTP headers, forwarded on the HTTP and LightPanda render paths (camofox renders do not apply them). A `User-Agent` here overrides the default. |
