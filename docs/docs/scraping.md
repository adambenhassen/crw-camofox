...
| `headers` | object | `{}` | Custom HTTP headers, forwarded on the HTTP and LightPanda render paths (camofox renders do not apply them). A `User-Agent` here overrides the default. On a LightPanda render they apply to every request the page makes (subresources included), so avoid putting cross-origin-sensitive credentials like `Authorization` here. |
