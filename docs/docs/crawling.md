...
| `headers` | object | `{}` | Custom HTTP headers applied to every page the crawl fetches, on the HTTP and LightPanda render paths (camofox renders do not apply them). A `User-Agent` here overrides the default. On a LightPanda render they apply to every request the page makes (subresources included), so avoid cross-origin-sensitive credentials like `Authorization` here |
