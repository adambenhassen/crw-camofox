...
| `engines` | string[] | no | Camofox backend only. Engine(s) to query, default `["google"]`, max 4. Valid: `google`, `bing`, `duckduckgo`, `github` (e.g. `["google","bing","duckduckgo"]`). Results are merged and deduped. Ignored by the SearXNG backend. |
