...
| `engines` | string[] | `["google"]` | **Camofox backend only.** Which engine(s) to search; results from multiple engines are merged and deduped by URL (a URL found by several engines ranks higher). Max 4. Valid values: `google`, `bing`, `duckduckgo`, `wikipedia`, `youtube`, `reddit`, `amazon`, `github`. Browser engines (everything except `github`) run **sequentially** on the warm tab, so N engines ≈ N× latency; `github` uses the GitHub REST Search API (set `[search].github_token` to lift its rate limit). Ignored by the SearXNG backend. |
