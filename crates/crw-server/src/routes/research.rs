...
//! Camofox web search (Google for recall, GitHub engine for code) merged with
...
use crate::state::{AppState, SearchBackend};
...
fn backend(state: &AppState) -> Result<SearchBackend, CrwError> {
    state.search.as_ref().cloned().ok_or_else(|| {
...
/// One Camofox web-search leg → arXiv-only [`PaperHit`]s. Camofox has no
/// scholarly engines (arXiv/Scholar/PubMed), so recall here is plain web search
/// (Google) for arXiv-ID discovery; the scholarly results come from the
/// OpenAlex + Semantic Scholar leg in [`search_papers`].
async fn camofox_papers(
...
pub async fn search_papers(
    ...
    let kz = keys(&state);
    // Camofox web search (Google) for arXiv-ID discovery + OpenAlex + SS, in
    // parallel. Upstream's second scholarly-engine SearXNG leg is dropped here:
    // Camofox has no arXiv/Scholar/PubMed engines, so OpenAlex + Semantic Scholar
    // carry scholarly recall.
    let (web, oa_ss) = tokio::join!(
        camofox_papers(&client, vec![SearchEngine::Google], &q.query),
        research::search_papers_pools(&kz, &q.query, k, &f),
    );
    let mut pools = vec![web];
    pools.extend(oa_ss);
...
/// `GET /v1/search/research/github` — GitHub search via Camofox's GitHub engine.
/// `ponytail:` it yields repo/readme hits, so `resultType` is `repo_readme`;
/// issue/PR/discussion granularity (Firecrawl's `github_history`) is a follow-up
/// if the engine exposes it.
pub async fn github(
    State(state): State<AppState>,
