...
fn test_app(camofox_url: &str) -> TestServer {
    // Search runs through the camofox REST endpoint; `mode = "none"` keeps the
    // enrichment scrape on the HTTP tier (see the module note).
    let toml = format!(
        r#"
[search]
enabled = true
...
/// Camofox search stub returning a single Google result that points back at
/// `page_url`, plus the page itself.
async fn mock_search_and_page(page_delay: Duration) -> MockServer {
    ...
    let rows = serde_json::to_string(&json!([
        { "url": page_url, "title": "slow page", "content": "snippet" }
    ]))
    .unwrap();
    for (p, body) in [
        ("/tabs", json!({ "tabId": "t1" })),
...
async fn enrichment_scrape_is_cut_at_the_caller_budget() {
    allow_loopback();
    let mock = mock_search_and_page(Duration::from_millis(2_000)).await;
...
async fn caller_timeout_raises_the_budget() {
    allow_loopback();
    let mock = mock_search_and_page(Duration::from_millis(300)).await;
    let server = test_app(&mock.uri());
