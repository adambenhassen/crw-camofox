...
async fn renderer() -> Arc<FallbackRenderer> {
    ...
    // The fork's renderer defaults to strict per-host politeness (cap = 1),
    // which would serialize the level through the host limiter and mask the
    // BFS-dispatch behavior this test asserts. Raise the cap the same way the
    // server does at startup (`with_host_limits` from CrawlerConfig).
    Arc::new(
        FallbackRenderer::new(&cfg, "crw-test", None, &StealthConfig::default())
            .expect("renderer builds in http-only mode")
            .with_host_limits(100.0, 5),
    )
}
