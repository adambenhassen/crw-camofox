...
//!
//! In the common self-host build — no camofox stealth tier, no
...
fn lightpanda_only() -> RendererConfig {
    RendererConfig {
...
fn site_block_suppression_tracks_whether_a_recovery_tier_exists() {
    ...
    assert!(
        ...
        "no stealth tier and no fallback proxy: a site block \
         must keep counting, or a blocked host costs 6-12x for zero recall"
    );

    // Only meaningful with `camofox`: a build without it cannot CONSTRUCT the
    // tier, so `has_recovery_tier` is correctly false there however the config
    // reads. That is the point of deriving it from built state.
    #[cfg(feature = "camofox")]
    {
        let with_stealth = RendererConfig {
        ...
        let r = FallbackRenderer::new(&with_stealth, "crw-test", None, &StealthConfig::default())
            .expect("renderer builds");
        assert!(
            r.has_recovery_tier(),
            "camofox can clear an IP-reputation block, so blocks stop poisoning the breaker"
        );
    }
    ...
    assert!(
        r.has_recovery_tier(),
        "a usable fallback proxy is a recovery egress even with no stealth tier"
    );
}
