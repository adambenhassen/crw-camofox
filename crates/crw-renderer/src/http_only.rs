    ...
    async fn fetch(
        ...
        let resp = loop {
            ...
            match send_result {
                ...
                // Switch to the fallback proxy for one more try. Arming does not consume
                // the transient retry budget (same policy as the 429 arm), so `!use_proxy`
                // bounds this to a single switch: worst case is direct, direct-retry,
                // [relaxed-TLS], proxy — each bounded by `deadline.remaining()`. Skipped
                // when too little budget remains for a proxy round trip to plausibly
                // complete; otherwise we pay a TCP setup only to be cancelled
                // milliseconds later.
                Ok(Err(e))
                    if !use_proxy
                        && self.ratelimit_proxy_client.is_some()
...
mod tests {
    ...
    /// fallback proxy and succeeds. Without the `is_egress_rejected` arm the
    /// request dies with `HttpError` and the caller sees a 502.
    #[cfg(unix)]
    #[tokio::test]
