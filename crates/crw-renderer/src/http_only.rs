...
pub struct HttpFetcher {
    ...
    /// or a proxy passed to [`Self::with_timeout`]). Egress provenance is not
...
impl PageFetcher for HttpFetcher {
    async fn fetch(
        ...
        let resp = loop {
            ...
            // once, rather than in the `Ok(Ok(_))` arms below, so no future arm can
            ...
            // This is the real predicate behind `direct_connect_failed`: an origin
            // answering 502-504 and then connect-timing-out on the retry must not be
            // reported as "no response from the origin".
            ...
            match send_result {
                        ...
                        && is_connection_failure(&e) =>
                {
                    ...
                    //
                    // `!origin_answered` is the third requirement: if the origin ever
                    // answered us with an HTTP status on this request, "no response from
                    // the origin" is provably false. Narrowed here at the source rather
                    // than at the arms that read it, so both inherit the same meaning.
                ...
                Ok(Err(e)) => {
                    ...
                    //
                    // `direct_connect_failed` is the one exception, the same rule the
                    // `Err(_)` arm above applies: it is set ONLY when the DIRECT egress
                    // hit a connect-phase TIMEOUT, a first-hand observation that the
                    // origin swallowed our SYNs. A proxy failure layered on top does not
                    // overturn that. Without this the class exits as `HttpError`, the
                    // ladder never short-circuits (`lib.rs` only stops on
                    // `TargetUnreachable`), and a host with no listening socket is handed
                    // every JS tier's full budget. A refused or reset proxy still lands
                    // at 502 because a refusal is not a timeout.
        ...
        let is_pdf = result.content_type.as_deref() == Some("application/pdf");
        ...
        //
        // Runs on the assembled result rather than inside the shared tail on
        // purpose: this is the only point where the decoded body and the egress
        // provenance (`use_proxy` / `has_static_proxy`) are both in scope.
        // Latching from the renderer would be unable to tell a direct block from
        // a proxied one, and a proxy-observed block would re-latch on every
        // request — the TTL would never expire, so direct would never be
        // re-probed and the host would be pinned to paid egress permanently.
        //
        ...
        if !use_proxy
            ...
            && !is_pdf
            && let Some(h) = &host
            && crate::detector::looks_like_generic_bot_wall(&result.html)
        ...
        // Stamped after the latch so the reported time covers the whole fetch,
        // as it did before the response tail was shared.
        Ok(FetchResult {
...
/// and tier-specific side effects (the egress latch stays in
/// `HttpFetcher::fetch`).
///
/// `tier_name` is the `rendered_with` label for non-PDF bodies (`"http"` or
/// `"impersonated-http"`); a PDF always reports `"pdf"`.
...
mod tests {
    ...
    /// false here — which is exactly why the predicate cannot be built on it.
    #[cfg(unix)]
    #[tokio::test]
