...
impl Backend {
    ...
    /// Embedded: only if a search backend (Camofox-direct or SearXNG) is
    /// configured, so a no-backend install doesn't advertise a tool that only
    /// returns `search_disabled`.
    fn search_available(&self) -> bool {
        match self {
            Backend::Proxy { .. } => true,
            #[cfg(feature = "embedded")]
            Backend::Embedded { state } => state.search.is_some(),
        }
    }
